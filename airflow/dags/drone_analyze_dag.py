"""STACD DAG — drone_analyze (single node).

Detection (Detectree2) + feature extraction/clustering (DINOv2 -> KMeans -> t-SNE)
in one blocking task that calls our backend's STACD-style compute endpoint.

Trigger (from our API):
    POST {AIRFLOW}/api/v1/dags/drone_analyze/dagRuns
    body: {"conf": {"project_id": "<uuid>", "run": <int>}}

The task posts to OUR backend:
    POST {DRONE_API_BASE}/api/v1/compute/analyze
    headers: Idempotency-Key = <dag_run_id>   (stable across retries -> replay, not recompute)
...
dag = DAG(
    ...
    description="Drone crown detection + DINOv2/KMeans/t-SNE clustering (single node)",
    schedule_interval=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["drone", "tree-crown", "analyze"],
...
def call_analyze(**context):
    ...
    print(f"[drone_analyze] POST {COMPUTE_URL} project={project_id} run={run} exec={execution_id}")
...
execute_analyze = PythonOperator(
    task_id="execute_drone_analyze",
    python_callable=call_analyze,
    dag=dag,
)
