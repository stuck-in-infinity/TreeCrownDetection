...
this pipeline actually has and returns a dict with
    ...
    code     - a specific machine code, not COMPUTE_FAILED
    message  - one sentence, plain language, no library jargon
    hint     - what to change, naming the real knob where there is one
    stage    - the pipeline stage that was running
    details  - the raw exception type and text, kept for engineers
...
#: Fallback code, unchanged from before, so existing consumers still work.
...
def _type_chain(exc: BaseException) -> str:
    ...
    Libraries wrap: rasterio raises its own error around an OSError, torch
    wraps CUDA failures. Matching only ``type(exc).__name__`` misses those.
    """
...
# Each rule: (code, match(exc, text, types) -> bool, message, hint)
# `message` and `hint` may be callables taking (exc, text) for a detail that is
# only knowable at failure time, e.g. which file was missing.
_RULES: list[tuple] = [
    (
        "GPU_OUT_OF_MEMORY",
        lambda e, t, ty: (
...
def _missing_path(exc: BaseException) -> str | None:
    ...
    Full server paths are not shown to users — they leak the storage layout and
    mean nothing to a forestry team. The basename is the part that identifies
    *which* artifact went missing, which is the useful half.
    """
...
def classify(exc: BaseException, stage: str | None = None) -> dict:
    """Interpret a pipeline exception. Never raises — the failure path must not fail.
    ...
    try:
        ...
        for code, match, message, hint in _RULES:
            ...
            except Exception:              # noqa: BLE001 - a bad rule must not mask the real error
                hit = False
            if hit:
                return _build(code,
                              ...
                              exc, stage)
        # Unrecognised. Still better than before: name the exception type and the
        # stage, so the message says *where* even when it cannot say *why*.
        return _build(
            GENERIC,
...
def _build(code, message, hint, exc, stage) -> dict:
    ...
    return {
        ...
        "hint": hint,
        "details": {
            # Kept verbatim. The classification is an interpretation laid on top
            # of the truth, never a replacement for it.
