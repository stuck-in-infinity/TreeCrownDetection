...
def drone_api(
    ...
    if airflow_enabled() and not body.execution_id:
        ...
        # Ortho selection, for the analyze action only. Validated here so a bad
        # ortho_id is a 400 from this call instead of a DAG that starts and then
        # fails, and PERSISTED onto project.params before the DAG is triggered:
        # the DAG's callback into run_analyze may not forward every conf key, and
        # a pin already on the project is read either way. conf carries it too,
        # additively — a DAG that ignores the key behaves exactly as before.
...
def run_analyze(
    ...
    # Resolve (and validate) the run's ortho before the claim, for the same
    # reason as the trigger: a bad selection must not leave a claimed job
    # behind. _apply_run_config below re-resolves and persists it.
